...
def _render_vss_index(domains: tuple[str, ...]) -> str:
    """Generate the HNSW vector-index build script per domain (mirrors _render_fts_index)."""
    lines = [
    ...
    for d in domains:
        lines.append(
            f"CREATE INDEX IF NOT EXISTS {d}_emb_hnsw ON {d}.embeddings "
...
def init_db() -> None:
    ...
    try:
        ...
        if domains:
            ...
            (SHARED_QUERIES / "vss_index.sql").write_text(
                _render_vss_index(domains), encoding="utf-8"
            )
    finally:
        con.close()
