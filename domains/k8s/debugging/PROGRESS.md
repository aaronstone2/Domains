...
**This leaf has 64 failure_modes** in the corpus (id-prefix `k8s.debugging.fm.*` or `k8s.debugging.*.fm.*`). Query via `pnpm harness lookup "<keyword>"` or `pnpm harness ask "<symptom>"`.
...
For interview-day relevance, this leaf is queryable through the harness — try `pnpm harness ask "<your-debugging-related-symptom>"` to see what's available.
