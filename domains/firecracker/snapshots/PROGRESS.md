# `firecracker/snapshots` — PROGRESS log
...
**This leaf has 12 failure_modes** in the corpus (id-prefix `firecracker.snapshots.fm.*` or `firecracker.snapshots.*.fm.*`). Query via `pnpm harness lookup "<keyword>"` or `pnpm harness ask "<symptom>"`.
...
For interview-day relevance, this leaf is queryable through the harness — try `pnpm harness ask "<your-snapshots-related-symptom>"` to see what's available.
