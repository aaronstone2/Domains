# `firecracker/networking` — PROGRESS log
...
**This leaf has 15 failure_modes** in the corpus (id-prefix `firecracker.networking.fm.*` or `firecracker.networking.*.fm.*`). Query via `pnpm harness lookup "<keyword>"` or `pnpm harness ask "<symptom>"`.
