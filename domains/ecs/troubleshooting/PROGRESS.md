# `ecs/troubleshooting` — PROGRESS log
...
**This leaf has 7 failure_modes** in the corpus (id-prefix `ecs.troubleshooting.fm.*` or `ecs.troubleshooting.*.fm.*`). Query via `pnpm harness lookup "<keyword>"` or `pnpm harness ask "<symptom>"`.
...
For interview-day relevance, this leaf is queryable through the harness — try `pnpm harness ask "<your-troubleshooting-related-symptom>"` to see what's available.
