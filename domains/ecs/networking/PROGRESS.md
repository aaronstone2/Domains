...
**This leaf has 8 failure_modes** in the corpus (id-prefix `ecs.networking.fm.*` or `ecs.networking.*.fm.*`). Query via `pnpm harness lookup "<keyword>"` or `pnpm harness ask "<symptom>"`.
