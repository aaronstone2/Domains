# Plan — `market/market-landscape`

## Leaf spec (Session 0)

- **Cluster:** market
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** FREEZE the canonical feature taxonomy (capability areas → sub-features, categories, layers, pain_score, kano_class, hci_relevance); define the 11-archetype map.
- **Primary tables:** `features`, `product_features`, `concepts`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `feature:feat.*`, `market:landscape.*`
- **Row estimate:** ~80-120 features, archetype concepts
- **Why this is its own leaf:** Kept whole — a single freezing authority is mandatory (contract C3). No company dependency: it is a capability vocabulary, authored before scorecards exist.
