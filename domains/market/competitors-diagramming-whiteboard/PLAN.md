# Plan — `market/competitors-diagramming-whiteboard`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Collaborative whiteboards + AI diagram tools (Figma = INTEGRATION PARTNER, flag relevant_to_us): Miro, Figma/FigJam, Excalidraw, tldraw, Whimsical, Napkin, Qlerify, Mural, MockFlow.
- **Primary tables:** `products`, `product_features`, `competitors`, `partners`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.diag-board.*`, `feature:comp.diag-board.feat.*`
- **Row estimate:** 9 products, ~55 product_features
- **Why this is its own leaf:** Second half of diagramming under cap; whiteboard authority distinct; Figma must be modeled as partner not competitor (coopetition).
