# Plan — `market/theory-mixed-initiative`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Mixed-initiative interaction (Horvitz 1999, 12 principles) + control allocation, autonomy levels, mode switching.
- **Primary tables:** `theory_concepts`, `claims`, `relationships`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.mixed-init.*`
- **Row estimate:** ~6 theory_concepts, ~10 claims
- **Why this is its own leaf:** Horvitz authority; merges area-2 and area-4 mixed-initiative duplicates into one leaf. Core to copilot-first hybrid → exhaustive.
