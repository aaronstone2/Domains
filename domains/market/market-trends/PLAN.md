# Plan — `market/market-trends`

## Leaf spec (Session 0)

- **Cluster:** market
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Cross-cutting trend signals: AI-native convergence, agentic 2026, consolidation (dbt+Fivetran), viz-only underfunding, graph-adoption friction.
- **Primary tables:** `market_metrics`, `claims`, `concepts`
- **Depends on:** `companies`, `market-landscape`, `market-sizing`
- **Claim categories (owned):** `market:trend.*`
- **Row estimate:** ~25 trend claims, ~15 metrics
- **Why this is its own leaf:** Synthesis-of-signals across archetypes — distinct from point sizing; needs landscape+sizing first to aggregate.
