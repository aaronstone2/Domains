# Plan — `market/theory-visual-programming-dataflow`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Visual/dataflow programming + the Deutsch limit (~50 primitives), end-user programming, why node-canvas builders hit a complexity ceiling.
- **Primary tables:** `theory_concepts`, `claims`, `relationships`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.visual-prog.*`
- **Row estimate:** ~6 theory_concepts, ~12 claims
- **Why this is its own leaf:** Central context for the wedge (why metro-map beats dense dataflow builders); distinct VPL authority → exhaustive.
