# Plan — `market/theory-graph-aesthetics-readability`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Graph aesthetics & readability: crossing minimization, symmetry, edge density, empirical cognitive-load studies (Huang, Kobourov).
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.graph-aesthetics.*`
- **Row estimate:** ~6 theory_concepts, ~10 claims
- **Why this is its own leaf:** Empirical human-factors authority distinct from layout algorithms; evidence base for 'metro-map reduces cognitive load' → exhaustive.
