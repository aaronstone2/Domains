# Plan — `market/competitors-graph-db-viz-native`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** DB-bundled/embedded graph explorers: Neo4j Bloom, Memgraph Lab, Kuzu Explorer, gDotV, NebulaGraph Explorer, SemSpect.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.gdbv-native.*`, `feature:comp.gdbv-native.feat.*`
- **Row estimate:** 6 products, ~60 product_features
- **Why this is its own leaf:** graph-db-visualization (16 cos) exceeds the 8-10 cap and splits cleanly on authority: engine-bundled explorers vs DB-agnostic platforms.
