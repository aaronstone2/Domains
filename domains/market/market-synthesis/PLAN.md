# Plan — `market/market-synthesis`

## Leaf spec (Session 0)

- **Cluster:** synthesis
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** The fully-cited research paper: whitespace thesis, segment attractiveness, differentiation, pricing gaps, theory-grounded wedge — reads all claims, writes zero.
- **Primary tables:** `reports`, `relationships`, `concepts`
- **Depends on:** `value-prop`, `business-model`, `market-trends`, `market-sizing`, `pricing`, `ux-teardown-graph-db-viz`, `ux-teardown-rag-agent-builders`, `theory-metro-map-metaphor`, `theory-visual-programming-dataflow`, `theory-hybrid-multimodal-interfaces`
- **Claim categories (owned):** `(read-only — writes reports + synthesis relationships only, no claim rows)`
- **Row estimate:** ~1 paper report, ~50 synthesis relationships
- **Why this is its own leaf:** Final read-only synthesis (contract C5 conflation guard): writes reports/synthesis relationships only, never claims; depends on the whole corpus.
