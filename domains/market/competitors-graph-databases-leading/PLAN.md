# Plan — `market/competitors-graph-databases-leading`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Leading property/graph engines (adjacency + partnership surface): Neo4j, TigerGraph, Memgraph, ArangoDB, Kuzu, FalkorDB.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.gdb-leading.*`, `feature:comp.gdb-leading.feat.*`
- **Row estimate:** 6 products, ~40 product_features
- **Why this is its own leaf:** graph-databases (12) > cap; established engines vs emerging/zero-ETL engines split. Adjacency → standard depth.
