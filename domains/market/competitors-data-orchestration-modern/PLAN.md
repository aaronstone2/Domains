# Plan — `market/competitors-data-orchestration-modern`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Modern data orchestrators with DAG/graph views: Airflow, Dagster, Prefect, Mage, Kestra, Astronomer, Orchestra.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.data-orch.*`, `feature:comp.data-orch.feat.*`
- **Row estimate:** 7 products, ~45 product_features
- **Why this is its own leaf:** Orchestrator slice; DAG-view UX is the adjacency. Kept <=cap; engine runtimes split off separately.
