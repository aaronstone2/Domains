# Plan — `market/theory-working-memory`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Working-memory capacity limits (Miller 7±2; Cowan 3-5); hard bound on simultaneous visible controls.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.wm.*`
- **Row estimate:** ~4 theory_concepts, ~6 claims
- **Why this is its own leaf:** Distinct seminal authority (Miller/Cowan) and a quantitative design bound; small but disjoint from CLT.
