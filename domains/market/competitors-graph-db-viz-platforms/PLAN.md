# Plan — `market/competitors-graph-db-viz-platforms`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** DB-agnostic enterprise platforms + commercial toolkits: Linkurious, Graphistry, GraphAware Hume, Kineviz GraphXR, Tom Sawyer, Cambridge Intelligence (KeyLines/ReGraph), yWorks (yFiles/yEd), Gephi, Graphileon, Graphlytic.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.gdbv-platform.*`, `feature:comp.gdbv-platform.feat.*`
- **Row estimate:** 10 products, ~100 product_features
- **Why this is its own leaf:** Second half of the 16-company direct set; <=10 cap; toolkit/SDK authority distinct from native explorers.
