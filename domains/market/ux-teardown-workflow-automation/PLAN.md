# Plan — `market/ux-teardown-workflow-automation`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** <=5 flows on Zapier, Make, n8n, Node-RED, Latenode (build-first-workflow) — canonical pane-bloat evidence.
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-workflow-automation-ipaas`, `competitors-workflow-automation-oss`, `market-landscape`
- **Claim categories (owned):** `ux:ux.wfa.*`, `hci:ux.wfa.*`
- **Row estimate:** ~25 screens, 5 flows, ~12 patterns
- **Why this is its own leaf:** Per-archetype teardown; canonical wedge-target HCI evidence → exhaustive, <=5 flows.
