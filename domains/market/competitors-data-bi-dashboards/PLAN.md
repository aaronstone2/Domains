# Plan — `market/competitors-data-bi-dashboards`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** BI dashboard/analytics platforms (data-engineer ICP consumption layer): Metabase, Superset, Tableau, Power BI, Looker, Grafana, Redash, QuickSight, OBIEE.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.bi-dash.*`, `feature:comp.bi-dash.feat.*`
- **Row estimate:** 9 products, ~70 product_features
- **Why this is its own leaf:** data-bi-notebook (18) > cap; dashboards/BI vs notebooks/AI-native split on user authority.
