# Plan — `market/competitors-db-schema-modeling-dev`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Developer/enterprise modelers + DB-IDE ERD: Vertabelo, SqlDBM, QuickDBD, ERDPlus, pgModeler, DBeaver EER.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.dbschema-dev.*`, `feature:comp.dbschema-dev.feat.*`
- **Row estimate:** 6 products, ~55 product_features
- **Why this is its own leaf:** Second half of db-schema-modeling under cap; dev-tool/enterprise authority distinct.
