# Plan — `market/ux-teardown-db-schema-modeling`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** <=5 flows on Azimutt, ChartDB, dbdiagram.io, DbSchema, Prisma Studio (live-DB connect → first ERD).
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-db-schema-modeling-visual`, `competitors-db-schema-modeling-dev`, `market-landscape`
- **Claim categories (owned):** `ux:ux.dbschema.*`, `hci:ux.dbschema.*`
- **Row estimate:** ~25 screens, 5 flows, ~10 patterns
- **Why this is its own leaf:** Per-archetype teardown, <=5 flows; direct ICP authority → exhaustive.
