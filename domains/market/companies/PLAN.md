# Plan — `market/companies`

## Leaf spec (Session 0)

- **Cluster:** spine
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Insert exactly one row per registry org (~158) + the MetroGraph self-row (is_self=TRUE); funding rounds + people stubs.
- **Primary tables:** `companies`, `funding_rounds`, `people`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `funding:co.*`
- **Row estimate:** ~160 companies, ~120 funding_rounds, ~30 people stubs
- **Why this is its own leaf:** Kept whole and FIRST: single authority for company facts; freezes company.id so all other leaves reference read-only. People is a designed stub.
