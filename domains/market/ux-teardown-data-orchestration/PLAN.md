# Plan — `market/ux-teardown-data-orchestration`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** <=5 flows on Airflow, Dagster, dbt Explorer, Prefect, Coalesce (DAG/lineage graph views).
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-data-orchestration-modern`, `competitors-data-transformation-quality`, `competitors-data-etl-ingestion`, `market-landscape`
- **Claim categories (owned):** `ux:ux.dataorch.*`, `hci:ux.dataorch.*`
- **Row estimate:** ~15 screens, 5 flows, ~8 patterns
- **Why this is its own leaf:** Per-archetype teardown; DAG/lineage-view adjacency → standard, <=5 flows.
