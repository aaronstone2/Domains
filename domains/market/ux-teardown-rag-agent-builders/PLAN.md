# Plan — `market/ux-teardown-rag-agent-builders`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** <=5 flows on Flowise, Langflow, Dify, Gumloop, Coze — agent-vs-graph-chat confusion documented.
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-rag-agent-builders-canvas`, `competitors-rag-agent-builders-conversational`, `market-landscape`
- **Claim categories (owned):** `ux:ux.rag.*`, `hci:ux.rag.*`
- **Row estimate:** ~25 screens, 5 flows, ~12 patterns
- **Why this is its own leaf:** Per-archetype teardown; named in wedge → exhaustive, <=5 flows.
