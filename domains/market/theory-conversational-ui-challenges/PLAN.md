# Plan — `market/theory-conversational-ui-challenges`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Conversational UI limitations: ambiguity, context, intent recognition — why pure chat fails for structured tasks.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.conv-ui.*`
- **Row estimate:** ~4 theory_concepts, ~8 claims
- **Why this is its own leaf:** Distinct conversational-AI literature; diagnoses the chat-abandonment starting thesis.
