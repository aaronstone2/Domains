# Plan — `market/competitors-data-transformation-quality`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Transformation (lineage DAG viz) + data quality/observability: dbt, SQLMesh, Dataform, Great Expectations, Soda, Elementary, Monte Carlo, Databand.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.data-transform.*`, `feature:comp.data-transform.feat.*`
- **Row estimate:** 8 products, ~50 product_features
- **Why this is its own leaf:** Transform+observability slice of data-orchestration; lineage-graph views are a relevant adjacency worth isolating.
