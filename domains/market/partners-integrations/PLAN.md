# Plan — `market/partners-integrations`

## Leaf spec (Session 0)

- **Cluster:** partners
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** MetroGraph's integration surface (Figma, Google Drive, graph-DB engines, modern data stack) + competitors' ecosystems; relevant_to_us flagging.
- **Primary tables:** `partners`, `relationships`, `claims`
- **Depends on:** `companies`
- **Claim categories (owned):** `integration:intg.*`
- **Row estimate:** ~60 partners, ~80 relationships
- **Why this is its own leaf:** Owns partners table (contract C10) + integrates_with/partners_with relationships; called-out integrations (Figma, Google Drive) are a distinct strategic authority.
