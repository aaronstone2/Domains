# Plan — `market/theory-instrumental-interaction`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Beaudouin-Lafon post-WIMP instrumental interaction (reification, polymorphism, reuse, currying) + AI-Instruments.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.instrumental.*`
- **Row estimate:** ~4 theory_concepts, ~6 claims
- **Why this is its own leaf:** Distinct post-WIMP authority; nodes/edges as reifiable instruments bridges DM + AI intent.
