# Plan — `market/competitors-db-schema-modeling-visual`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Visual/cloud/live-DB ERD tools (direct ICP rivals): Azimutt, ChartDB, dbdiagram.io, DrawSQL, DbSchema, Prisma Studio, ER Flow.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.dbschema-visual.*`, `feature:comp.dbschema-visual.feat.*`
- **Row estimate:** 7 products, ~70 product_features
- **Why this is its own leaf:** db-schema-modeling (13 cos) > cap; splits on authority: modern visual/AI ERD vs classic dev/enterprise modelers.
