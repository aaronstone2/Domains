# Plan — `market/theory-database-schema-visualization`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** DB schema & ERD visualization theory: Chen ER model, relational/noSQL/graph schema representation, large-schema complexity.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.db-schema-viz.*`
- **Row estimate:** ~5 theory_concepts, ~10 claims
- **Why this is its own leaf:** Direct application domain (ICP); Chen ERD lineage is a distinct authority → exhaustive.
