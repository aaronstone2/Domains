# Plan — `market/theory-choice-cost`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Cost of choices: Hick-Hyman law (RT=a+b·log2 n) + Paradox of Choice / decision fatigue (Schwartz).
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.choice-cost.*`
- **Row estimate:** ~5 theory_concepts, ~10 claims
- **Why this is its own leaf:** Two related authorities (Hick/Hyman motor-RT, Schwartz behavioral-econ) merged under one 'cost of options' theme central to the surface-area wedge; merge avoids two thin leaves while noting the distinct literatures.
