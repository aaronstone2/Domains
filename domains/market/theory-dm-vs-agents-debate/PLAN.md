# Plan — `market/theory-dm-vs-agents-debate`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Shneiderman-Maes debate (DM vs intelligent agents) + modern prompt-vs-direct-manipulation + copilot-vs-autonomous-agent distinctions.
- **Primary tables:** `theory_concepts`, `claims`, `relationships`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.dm-vs-agents.*`
- **Row estimate:** ~6 theory_concepts, ~12 claims
- **Why this is its own leaf:** Merges three overlapping subareas (the classic debate, prompt-vs-DM, copilot-vs-agent) into the single 'control vs delegation' authority. Directly frames 'best of both' → exhaustive.
