# Plan — `market/theory-nl-interfaces-structured-data`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** NL→structured-output (NL2SQL/NL2VIS): ambiguity resolution, DataTone, pragmatics, interactive disambiguation.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.nl-structured.*`
- **Row estimate:** ~5 theory_concepts, ~10 claims
- **Why this is its own leaf:** MetroGraph-specific (NL→graph structure) with its own NL2VIS/NL2SQL corpus → exhaustive.
