# Plan — `market/competitors-diagramming-as-code`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Diagram-as-code + ERD/graph-capable diagrammers: Mermaid, Graphviz, PlantUML, D2, Lucidchart, draw.io, Creately, Penrose, Visio.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.diag-code.*`, `feature:comp.diag-code.feat.*`
- **Row estimate:** 9 products, ~65 product_features
- **Why this is its own leaf:** diagramming (18) > cap; diagram-as-code/ERD-render vs collaborative whiteboard split on authority.
