# Plan — `market/competitors-data-etl-ingestion`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** ETL/ELT ingestion + visual ELT + agentic migration: Fivetran, Airbyte, Talend, Matillion, Coalesce, dltHub, Bruin, Rubie.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.data-etl.*`, `feature:comp.data-etl.feat.*`
- **Row estimate:** 8 products, ~55 product_features
- **Why this is its own leaf:** data-orchestration (34) split 5-way; ingestion authority disjoint from transform/orchestrate/warehouse.
