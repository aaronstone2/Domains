# Plan — `market/competitors-data-warehouse-lakehouse`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `scout`; overridden to exhaustive per directive)
- **Scope:** Warehouse/lakehouse platform context for the ICP stack: Databricks, Snowflake, ClickHouse, Dremio.
- **Primary tables:** `products`, `competitors`, `companies`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.data-warehouse.*`
- **Row estimate:** 4 products, ~15 product_features
- **Why this is its own leaf:** Platform context (not viz rivals) — scout depth; 4 mega-vendors kept whole.
