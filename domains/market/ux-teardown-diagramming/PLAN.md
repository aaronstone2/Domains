# Plan — `market/ux-teardown-diagramming`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** <=5 flows on Lucidchart, draw.io, Miro, Figma/FigJam, Mermaid (ERD/graph render) — Figma flow as partner-embed study.
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-diagramming-as-code`, `competitors-diagramming-whiteboard`, `market-landscape`
- **Claim categories (owned):** `ux:ux.diag.*`, `hci:ux.diag.*`
- **Row estimate:** ~15 screens, 5 flows, ~8 patterns
- **Why this is its own leaf:** Per-archetype teardown; adjacency + Figma partner-embed surface → standard, <=5 flows.
