# Plan — `market/customer-segments`

## Leaf spec (Session 0)

- **Cluster:** customers
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** FREEZE segment.id (noSQL/SQL startups, data engineers, analysts, etc.); personas + the VPC customer profile (jobs/pains/gains) with severity/importance/relief.
- **Primary tables:** `segments`, `personas`, `jobs_pains_gains`
- **Depends on:** `companies`
- **Claim categories (owned):** `segment:seg.*`, `demand:seg.demand.*`
- **Row estimate:** ~8 segments, ~12 personas, ~60 jobs_pains_gains
- **Why this is its own leaf:** Kept whole and authoritative — freezes segment.id (contract C7) and owns the customer-profile side of VPC so value-prop can map onto it without write contention.
