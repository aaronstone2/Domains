# Plan — `market/competitors-rag-agent-builders-conversational`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Conversational / enterprise / NL-first agent builders: Voiceflow, Coze, Wordware, Lindy, Airia, Botpress, Rasa, Copilot Studio, Vertex AI Agent Builder, Relay.app.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.rag-conv.*`, `feature:comp.rag-conv.feat.*`
- **Row estimate:** 10 products, ~80 product_features
- **Why this is its own leaf:** Second half of rag-agent-builders under cap; conversational/enterprise authority distinct from visual canvas.
