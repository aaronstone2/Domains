# Plan — `market/value-prop`

## Leaf spec (Session 0)

- **Cluster:** vpc
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** MetroGraph value-map: pain relievers / gain creators / products-and-services mapped to frozen jobs/pains/gains; per-competitor we_win_on differentiation; whitespace claim.
- **Primary tables:** `jobs_pains_gains`, `relationships`, `claims`
- **Depends on:** `customer-segments`, `market-landscape`, `competitors-graph-db-viz-native`, `competitors-graph-db-viz-platforms`, `competitors-db-schema-modeling-visual`, `ux-teardown-graph-db-viz`, `theory-metro-map-metaphor`, `theory-visual-programming-dataflow`, `theory-hybrid-multimodal-interfaces`
- **Claim categories (owned):** `demand:vp.*`, `competition:vp.diff.*`
- **Row estimate:** ~30 relief mappings, ~20 differentiation claims
- **Why this is its own leaf:** Synthesis-of-positioning leaf — needs frozen segments + competitor we_win_on/they_win_on + theory; writes value-map relationships (grounded_in/substitute_for) and the headline whitespace claim.
