# Plan — `market/theory-hybrid-multimodal-interfaces`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Hybrid NL + direct-manipulation / multimodal interfaces (Graphologue, LLM-GUI architectures) — the space MetroGraph occupies.
- **Primary tables:** `theory_concepts`, `claims`, `relationships`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.hybrid-mm.*`
- **Row estimate:** ~5 theory_concepts, ~12 claims
- **Why this is its own leaf:** CORE thesis space; emerging-but-distinct corpus → exhaustive.
