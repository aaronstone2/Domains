# Plan — `market/ux-teardown-graph-db-viz`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Deep flows (<=5) on the direct set: Neo4j Bloom, Linkurious, Kineviz GraphXR, Memgraph Lab, Gephi — pane_count/click_depth/hci_cost.
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-graph-db-viz-native`, `competitors-graph-db-viz-platforms`, `market-landscape`
- **Claim categories (owned):** `ux:ux.gdbv.*`, `hci:ux.gdbv.*`
- **Row estimate:** ~25 screens, 5 flows, ~10 patterns
- **Why this is its own leaf:** One teardown per archetype; capped at 5 deep flows. Direct-competitor authority → exhaustive; depends on its competitor leaves for product.id + feature linkage.
