# Plan — `market/ux-teardown-data-bi-notebook`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** <=5 flows on Metabase, Superset, Hex, Tableau, Grafana (query/explore-to-dashboard).
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-data-bi-dashboards`, `competitors-data-notebooks-ai`, `market-landscape`
- **Claim categories (owned):** `ux:ux.bi.*`, `hci:ux.bi.*`
- **Row estimate:** ~15 screens, 5 flows, ~8 patterns
- **Why this is its own leaf:** Per-archetype teardown; ICP-adjacent → standard, <=5 flows.
