# Plan — `market/theory-recognition-recall`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Recognition vs recall (Nielsen heuristic #6): visible options cheaper than hidden commands.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.recog-recall.*`
- **Row estimate:** ~3 theory_concepts, ~5 claims
- **Why this is its own leaf:** Distinct usability-heuristic authority (Nielsen); grounds the visibility thesis.
