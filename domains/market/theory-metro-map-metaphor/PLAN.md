# Plan — `market/theory-metro-map-metaphor`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Metro-map metaphor for abstract data: octilinear schematic layout, MetroSets, applications to software/pathways/concepts.
- **Primary tables:** `theory_concepts`, `claims`, `relationships`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.metro-map.*`
- **Row estimate:** ~6 theory_concepts, ~12 claims
- **Why this is its own leaf:** MetroGraph's CORE differentiator and primary visual metaphor → exhaustive, standalone authority.
