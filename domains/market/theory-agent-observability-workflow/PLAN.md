# Plan — `market/theory-agent-observability-workflow`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Visualizing agent workflows/observability: DAG/Sankey overlays, per-step latency, human-in-the-loop interruption points.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.agent-obs.*`
- **Row estimate:** ~4 theory_concepts, ~6 claims
- **Why this is its own leaf:** Distinct observability authority; metro-map↔DAG mapping is a specific design opportunity worth its own leaf.
