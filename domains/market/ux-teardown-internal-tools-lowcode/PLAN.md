# Plan — `market/ux-teardown-internal-tools-lowcode`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** <=5 flows on Retool, Appsmith, Budibase, Airtable, NocoDB (config sprawl + data-frontend).
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-internal-tools-lowcode`, `competitors-lowcode-app-platforms`, `competitors-nocode-databases`, `market-landscape`
- **Claim categories (owned):** `ux:ux.itlc.*`, `hci:ux.itlc.*`
- **Row estimate:** ~25 screens, 5 flows, ~12 patterns
- **Why this is its own leaf:** Per-archetype teardown spanning all three internal-tools sub-leaves; config-sprawl wedge → exhaustive, <=5 flows.
