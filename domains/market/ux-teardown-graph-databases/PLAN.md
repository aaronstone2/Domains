# Plan — `market/ux-teardown-graph-databases`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** <=5 flows on bundled UIs: Neo4j Browser, Memgraph, TigerGraph, NebulaGraph, PuppyGraph (query-to-viz).
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-graph-databases-leading`, `competitors-graph-databases-emerging`, `market-landscape`
- **Claim categories (owned):** `ux:ux.gdb.*`, `hci:ux.gdb.*`
- **Row estimate:** ~15 screens, 5 flows, ~8 patterns
- **Why this is its own leaf:** Per-archetype teardown; adjacency → standard, <=5 flows.
