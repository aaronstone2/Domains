# Plan — `market/competitors-nocode-databases`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** No-code database/spreadsheet frontends (ICP-adjacent data frontends over SQL/noSQL): Airtable, NocoDB, Teable, Baserow, Grist, Knack, Smartsheet.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.itlc-nocodedb.*`, `feature:comp.itlc-nocodedb.feat.*`
- **Row estimate:** 7 products, ~55 product_features
- **Why this is its own leaf:** Third slice of internal-tools-lowcode; data-frontend ICP adjacency is a distinct competitive sub-question.
