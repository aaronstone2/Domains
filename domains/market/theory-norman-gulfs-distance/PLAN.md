# Plan — `market/theory-norman-gulfs-distance`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** Norman's gulfs of execution/evaluation, affordances/signifiers, + Hutchins semantic/articulatory distance.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.norman.*`
- **Row estimate:** ~6 theory_concepts, ~8 claims
- **Why this is its own leaf:** Norman/Hutchins authority; affordances and semantic-articulatory distance merged here (same author lineage) rather than as thin separate leaves.
