# Plan — `market/competitors-rag-agent-builders-canvas`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Visual canvas RAG/agent builders (named wedge: agent-vs-graph-chat confusion): Flowise, Langflow, Dify, Gumloop, Rivet, BuildShip, MindStudio, RAGFlow, Mastra, AnythingLLM.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.rag-canvas.*`, `feature:comp.rag-canvas.feat.*`
- **Row estimate:** 10 products, ~90 product_features
- **Why this is its own leaf:** rag-agent-builders (20) > cap; canvas/visual builders vs conversational/enterprise agents split. Exhaustive: directly named in wedge.
