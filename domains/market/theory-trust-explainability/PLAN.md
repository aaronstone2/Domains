# Plan — `market/theory-trust-explainability`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Trust, explainability & transparency in AI UIs; absorbs RAG-UX (showing retrieval context) and model-card documentation.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.trust-xai.*`
- **Row estimate:** ~6 theory_concepts, ~10 claims
- **Why this is its own leaf:** Distinct XAI/trust authority; merges RAG-UX + model-cards (judged non-leaf) here. Core to agent-vs-graph-chat confusion fix → exhaustive.
