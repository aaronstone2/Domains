# Plan — `market/theory-cognitive-load`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Cognitive Load Theory (Sweller/Ayres/Kalyuga): intrinsic/extraneous/germane load; absorbs progressive disclosure + information architecture as applied CLT.
- **Primary tables:** `theory_concepts`, `claims`, `relationships`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `hci:thy.cog-load.*`, `theory:thy.cog-load.*`
- **Row estimate:** ~8 theory_concepts, ~12 claims
- **Why this is its own leaf:** Foundational theory with its own seminal corpus; absorbs the non-leaf progressive-disclosure pattern. Core to surface-area wedge → exhaustive.
