# Plan — `market/market-sizing`

## Leaf spec (Session 0)

- **Cluster:** market
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** TAM/SAM/SOM + CAGR for DB-viz / graph-viz / data-engineer tooling, top-down and bottom-up, per whole-market and per-archetype.
- **Primary tables:** `market_metrics`, `reports`, `segments`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `market:size.*`
- **Row estimate:** ~40 market_metrics, ~15 sizing reports
- **Why this is its own leaf:** Separate authority (analyst sizing reports) disjoint from feature/competitive sources; isolates every market number behind reports/market_metrics.
