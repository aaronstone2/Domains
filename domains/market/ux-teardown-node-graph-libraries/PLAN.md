# Plan — `market/ux-teardown-node-graph-libraries`

## Leaf spec (Session 0)

- **Cluster:** ux
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `standard`; overridden to exhaustive per directive)
- **Scope:** <=5 flows on canonical demo apps: React Flow, Cytoscape.js, Sigma.js, AntV G6, Rete.js.
- **Primary tables:** `ux_screens`, `ux_flows`, `ux_patterns`
- **Depends on:** `competitors-node-graph-libs-rendering`, `competitors-node-graph-libs-editors`, `market-landscape`
- **Claim categories (owned):** `ux:ux.nodelib.*`, `hci:ux.nodelib.*`
- **Row estimate:** ~15 screens, 5 flows, ~8 patterns
- **Why this is its own leaf:** Per-archetype teardown; foundation-layer → standard depth, <=5 flows.
