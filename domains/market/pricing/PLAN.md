...
- **Scope:** Pricing models + tier ladders for all priced products (reads product.id across competitor leaves); pricing-gap analysis vs MetroGraph.
- **Primary tables:** `pricing_models`, `pricing_tiers`, `claims`
- **Depends on:** `companies`, `market-landscape`, `competitors-graph-db-viz-native`, `competitors-db-schema-modeling-visual`, `competitors-workflow-automation-ipaas`, `competitors-internal-tools-lowcode`
- **Claim categories (owned):** `pricing:price.*`
- **Row estimate:** ~70 pricing_models, ~200 pricing_tiers
- **Why this is its own leaf:** Owns pricing_models/pricing_tiers (contract C9); pricing-page authority disjoint from feature/UX sources. Reads products created by competitor leaves (wave-ordered after them).
