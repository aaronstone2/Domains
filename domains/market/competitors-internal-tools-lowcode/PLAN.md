# Plan — `market/competitors-internal-tools-lowcode`

## Leaf spec (Session 0)

- **Cluster:** competitive
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Low-code internal-tool builders (config-sprawl wedge target): Retool, Appsmith, ToolJet, Budibase, Superblocks, UI Bakery, Illa, Lowdefy, Refine, NocoBase.
- **Primary tables:** `products`, `product_features`, `competitors`
- **Depends on:** `companies`, `market-landscape`
- **Claim categories (owned):** `competition:comp.itlc-builders.*`, `feature:comp.itlc-builders.feat.*`
- **Row estimate:** 10 products, ~90 product_features
- **Why this is its own leaf:** internal-tools-lowcode (25) split 3-way on authority: internal-tool builders / app platforms / no-code DBs. Exhaustive: Retool config-sprawl is core wedge evidence.
