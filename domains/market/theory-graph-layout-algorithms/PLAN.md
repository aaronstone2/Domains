# Plan — `market/theory-graph-layout-algorithms`

## Leaf spec (Session 0)

- **Cluster:** theory
- **Depth:** exhaustive (Session-0 risk-weighted suggestion was `exhaustive`; overridden to exhaustive per directive)
- **Scope:** Graph layout: force-directed (Eades, Fruchterman-Reingold), hierarchical (Sugiyama), tree (Reingold-Tilford), orthogonal/octilinear.
- **Primary tables:** `theory_concepts`, `claims`
- **Depends on:** _(none — wave 1)_
- **Claim categories (owned):** `theory:thy.graph-layout.*`
- **Row estimate:** ~8 theory_concepts, ~10 claims
- **Why this is its own leaf:** Large algorithmic corpus with distinct graph-drawing authorities; underpins metro-map octilinear routing → exhaustive. Absorbs hierarchical/tree (judged non-leaf).
