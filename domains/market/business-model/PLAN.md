...
- **Scope:** MetroGraph's 9-block Business Model Canvas (+ optionally key competitors' BMCs), linking segments/features/partners/pricing tiers.
- **Primary tables:** `bmc_blocks`, `claims`
- **Depends on:** `customer-segments`, `value-prop`, `pricing`, `partners-integrations`
- **Claim categories (owned):** `gtm:bmc.*`
- **Row estimate:** ~9-18 bmc_blocks, ~15 gtm claims
- **Why this is its own leaf:** Owns bmc_blocks; depends on value-prop, pricing, partners to link blocks. Distinct GTM authority.
